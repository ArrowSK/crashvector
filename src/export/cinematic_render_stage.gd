...
func _build_scene() -> void:
	_build_road()
	primary_actor = _build_vehicle_actor(scenario.primary_type, scenario.car_mass_kg, scenario.car_speed_kmh, scenario.car_position_m, scenario.car_heading_deg, scenario.car_preset_id, profile.primary_paint_id)
	primary_actor.name = "RenderPrimary"
	add_child(primary_actor)

	if ScenarioConfig.is_vehicle_actor_id(scenario.target_type):
		target_actor = _build_vehicle_actor(scenario.target_type, scenario.target_mass_kg, scenario.target_speed_kmh, scenario.target_position_m, scenario.target_heading_deg, scenario.target_car_preset_id, profile.target_paint_id)
		target_actor.name = "RenderTarget"
...
func _build_vehicle_actor(actor_type: StringName, mass_kg: float, speed_kmh: float, position_m: Vector3, heading_deg: float, passenger_preset_id: StringName, paint_id: StringName) -> Node3D:
	var actor := VehicleActorFactory.create(actor_type, mass_kg, speed_kmh, position_m, heading_deg, false, passenger_preset_id)
	if actor is CompactHatchback:
		(actor as CompactHatchback).paint_id = paint_id
	return actor
...
func _build_overlay() -> void:
	...
	title_label.text = "%s\n%s vs %s" % [scenario.title, _primary_display_name(), ScenarioConfig.target_display_name(scenario.target_type)]
...
func _apply_primary_frame(frame: Dictionary) -> void:
	_apply_actor_frame(primary_actor, frame.get("primary_state", {}), frame.get("primary_visual_state", {}))
...
func _apply_target_frame(frame: Dictionary) -> void:
	...
	if target_actor != null:
		_apply_actor_frame(target_actor, state, frame.get("target_visual_state", {}))
...
func _apply_actor_frame(actor: Node3D, state: Variant, visual_state: Variant) -> void:
	if actor == null or not (state is Dictionary):
		return
	var model_value: Variant = actor.get("model")
	if model_value is StructuralModel:
		StructuralSnapshot.apply(model_value, state)
	if actor.has_method("step_external"):
		actor.call("step_external", 0.0)
	if visual_state is Dictionary and actor.has_method("apply_replay_visual_state"):
		actor.call("apply_replay_visual_state", visual_state)
...
func _update_overlay(frame: Dictionary, output_time_s: float, replay_time_s: float) -> void:
	...
	hud_label.text = "%s\n%.0f km/h • front crush %.0f mm • t %.2f s%s" % [_primary_display_name(), speed, crush_mm, replay_time_s, slow_tag]

func _primary_display_name() -> String:
	if scenario.primary_type == ScenarioConfig.TARGET_PASSENGER_CAR:
		return PassengerCarCatalog.display_name(scenario.car_preset_id)
	return ScenarioConfig.actor_display_name(scenario.primary_type)
