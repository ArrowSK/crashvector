...
# / tree targets can yield permanently at high collision demand. M15 upgrades
# the same RoadUserRigidProxy3D API to articulated pedestrian and bicycle rigs.
...
func _replace_legacy_road_user_with_rigid_proxy() -> void:
	...
	if status_label != null:
		status_label.text = "Articulated rigid-body road-user preview — press Simulate"
	_update_metrics()
...
func _physics_process(delta: float) -> void:
	...
	if road_user_proxy != null and is_instance_valid(road_user_proxy):
		if car.rigid_chassis.front_crush_overlap_active():
			...
			if road_user_proxy.owns_collider(collider):
...
func _dispose_road_user_proxy() -> void:
	if road_user_proxy == null or not is_instance_valid(road_user_proxy):
		...
		return
	# The inherited road-user variables point at hidden structural-reference
	# children of this proxy. Clear those first so historical cleanup never tries
	# to remove a grandchild directly from the editor root.
...
func _road_user_rigid_metrics() -> Dictionary:
	...
	var velocity := road_user_proxy.center_of_mass_velocity_ms()
	var mass_kg := road_user_proxy.target_mass_kg
	return {
		"mass_kg": mass_kg,
		"linear_velocity_ms": velocity,
		"speed_kmh": PhysicsMetrics.ms_to_kmh(velocity.length()),
		"momentum_kg_ms": velocity * mass_kg,
		"kinetic_energy_j": 0.5 * mass_kg * velocity.length_squared(),
...
func _refresh_analysis_overlay() -> void:
	if road_user_proxy == null or not is_instance_valid(road_user_proxy):
		...
		return
	# The road-user structural-reference graph is local to the articulated target.
	# Until the vector overlay learns per-body transforms, show authoritative car
