...
The current verified public desktop release is `0.8.0-beta.3`. Current `main` contains source work through M22, but M19-M22 were committed while GitHub-hosted Actions capacity was exhausted and therefore remain runtime-unvalidated. Until the current source passes the required Godot, visual and native package gates, the canonical version and public download/update channel intentionally remain on `0.8.0-beta.3`.
...
Because `main` is currently ahead of the last verified public package, a new release must not be created merely by changing the version number.
...
4. Only after those gates are acceptable, increment `application/config/version` in `project.godot` and add matching human-readable notes under `docs/releases/<version>.md`.
