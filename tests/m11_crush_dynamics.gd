...
func _test_symmetric_wall_crush_shape(failures: Array[String]) -> void:
	...
	var final_front_length := _front_structure_length(model)
	var front_crush := initial_front_length - final_front_length
	if front_crush < 0.28:
		failures.append("M11 50 km/h wall impact did not materially shorten the front structure: %.3f m" % front_crush)
	...
	var yaw_deg := absf(_reference_yaw_deg(model))
	if yaw_deg > 1.5:
		failures.append("M11 centred wall impact generated artificial vehicle yaw: %.3f deg" % yaw_deg)
...
func _test_dynamic_pair_multipoint_contact(failures: Array[String]) -> void:
	...
	if absf(_reference_yaw_deg(car)) > 3.0:
		failures.append("M11 symmetric rear-truck impact generated excessive artificial car yaw")
...
func _test_high_speed_stability(failures: Array[String]) -> void:
	...
	var front_crush := initial_front_length - _front_structure_length(model)
	if front_crush < 0.40:
		failures.append("M11 140 km/h scenario failed to show substantial front collapse: %.3f m" % front_crush)
	...
	if absf(_reference_yaw_deg(model)) > 3.0:
		failures.append("M11 symmetric 140 km/h wall impact generated excessive yaw")
